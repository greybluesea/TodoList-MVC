...
/*
fmt.Println(os.Args)
	argsExceptPath := os.Args[1:]

	var IntArgsExceptPath = []int{}
	for _, arg := range argsExceptPath {
		integer, err := strconv.Atoi(arg)
		if err != nil {
			log.Fatal(err)
		}
		IntArgsExceptPath = append(IntArgsExceptPath, integer)
	}

	max := 0
	for _, integer := range IntArgsExceptPath {
		if integer > max {
			max = integer
		}
	}
